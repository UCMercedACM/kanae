...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript from `~/.claude/projects/<encoded-cwd>/<session-id>.jsonl`, where `<encoded-cwd>` is the working directory with `/` replaced by `-`. Don't glob across all of `~/.claude/projects/*/`; that reads unrelated private chats from other projects. Walk the log against what actually happened:
