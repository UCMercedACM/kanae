...
After the explanation is complete, spawn one architectural critic per model in `fable`, `opus`, `sonnet`, all in a single message.

For each critic:
- `subagent_type`: `Explore`
- `model`: one of `fable`, `opus`, `sonnet`. Escalate to `opus` when the architecture warrants deeper analysis.
