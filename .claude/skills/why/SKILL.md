...
Before spawning investigators, list the available MCP servers for this session. Check which MCP-backed tools appear in your available tools (via ToolSearch or the tool list), or run `claude mcp list` in the shell to see configured servers directly.
...
Use `general-purpose`, not `Explore` — MCP-backed investigators need their MCP tools available, and `Explore`'s restricted toolset isn't guaranteed to include them. Investigators still shouldn't write anything. That's a posture, not a sandbox.
...
Use `general-purpose`, not `Explore` — the synthesizer's quality check spot-verifies citations, which can require MCP access that `Explore`'s toolset may not include.
