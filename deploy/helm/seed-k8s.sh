...
BORG_SECRET=${BORG_SECRET:-kanae-borg}
...
# -o never reads the cluster, so it cannot keep existing values the way the
# apply path does: every run mints a fresh set. Overwriting a file that already
# holds a deployment's secrets would therefore hand you a NEW
# KRATOS_SECRETS_CIPHER, and Kratos data encrypted under the old one becomes
# unreadable — the exact failure a backup does not save you from.
if [[ -n $OUT_FILE && -e $OUT_FILE ]]; then
	abort "$OUT_FILE exists. This would generate entirely new secrets, not update it.
       To change a value, edit it in place with 'sops $OUT_FILE'.
       To deliberately rotate everything, delete the file first — and understand
       that identities encrypted under the current KRATOS_SECRETS_CIPHER are lost."
fi

# ---------------------------------------------------------------- helpers ---

# Read one key out of an existing Secret. Empty string when absent.
existing_from() {
	kubectl -n "$NAMESPACE" get secret "$1" \
		-o "jsonpath={.data.$2}" 2>/dev/null | base64 -d 2>/dev/null || true
}

existing() { existing_from "$ENV_SECRET" "$1"; }

# Keep what is already there unless -r. $2 is the generator command; $3 is the
# Secret to read back from, defaulting to the env Secret.
keep_or_generate() {
	local key=$1 gen=$2 secret=${3:-$ENV_SECRET} current=''
	[[ -n $OUT_FILE ]] || current=$(existing_from "$secret" "$key")
...
prompt_secret() {
	local key=$1 desc=$2 secret=${3:-$ENV_SECRET} current=''
	[[ -n $OUT_FILE ]] || current=$(existing_from "$secret" "$key")
	if [[ -n $current ]]; then
		log "$key already set, keeping it"
		printf '%s' "$current"
...
# Borg. Separate Secret because the backup CronJob is the only thing that reads
# it, and it is the one credential set whose loss is unrecoverable in the other
# direction: without BORG_PASSPHRASE the archives are just noise.
BORG_PASSPHRASE=$(keep_or_generate BORG_PASSPHRASE 'openssl rand -hex 32' "$BORG_SECRET")
AWS_ACCESS_KEY_ID=$(prompt_secret AWS_ACCESS_KEY_ID 'borg repo S3 access key id' "$BORG_SECRET")
AWS_SECRET_ACCESS_KEY=$(prompt_secret AWS_SECRET_ACCESS_KEY 'borg repo S3 secret key' "$BORG_SECRET")

# ----------------------------------------------------------- config.yml -----

log "rendering config.yml from ${CONFIG_DIST##*/}"

# config.dist.yml ships the Vite dev server as the CORS origin, which is right
# for a laptop and wrong for a deployment: the SPA is on Cloudflare Pages at
# ucmacm.dev, and left at the dist default every browser request from it is
# refused. Must match kanae.allowedOrigins in the chart's values.yaml — the
# chart renders this same file from those values, and the two paths producing
...
if [[ -n $OUT_FILE ]]; then
	umask 077
	{
		...
		printf '#\n'
		printf '# Feeds the chart directly:\n'
		printf '#   helm secrets install kanae ./deploy/helm/kanae -n kanae -f %s\n' "${OUT_FILE##*/}"
		printf 'secrets:\n'
		printf '  # Renders kanae-env / kanae-config / kanae-borg from the values below\n'
		printf '  # instead of mounting Secrets this script applied to the cluster.\n'
		printf '  create: true\n'
		printf '  dbUsername: %s\n' "$DB_USERNAME"
		...
		printf '  borgPassphrase: %s\n' "$BORG_PASSPHRASE"
		printf '  borgS3AccessKey: %s\n' "$AWS_ACCESS_KEY_ID"
		printf '  borgS3SecretKey: %s\n' "$AWS_SECRET_ACCESS_KEY"
	} >"$OUT_FILE"
...
log "applying $BORG_SECRET"
kubectl -n "$NAMESPACE" create secret generic "$BORG_SECRET" \
	--from-literal=BORG_PASSPHRASE="$BORG_PASSPHRASE" \
