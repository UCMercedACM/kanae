...
`deploy/kubernetes/src/templates/kanae.yml` shows up in a pull request as a
...
Files in `dist/` are named `<kind>-<name>.yml` by the render, one resource
...
- [x] Pin the versions in `mise.toml` that currently say `latest`: `helm`,
...
- [x] Add `yamllint` to `pyproject.toml` and write `.yamllint.yml`. Switch off every
...
- [x] Add the mise tasks `k8s:fmt`, `k8s:fmt:check`, and `k8s:lint`, matching
      the shape of the existing `scripts:format` and `scripts:lint` tasks.
- [x] Write `deploy/kubernetes/k3d.yml`, a k3d cluster config file. Put the
...
- [x] Disable the bundled Traefik in that config with `--disable=traefik`. k3s
...
- [x] Pick the hostname Phase 8's Gateway will answer to and write it in
      `deploy/kubernetes/k3d.yml` as a comment. k3d has no field for this, and
...
- [x] Add the mise task `k8s:up`. It creates the cluster from that file and then
      waits until `kubectl get nodes` reports Ready.
- [x] Add the mise task `k8s:down`. It deletes the cluster and waits for the
      deletion to finish before returning.
- [x] Add the mise task `k8s:reset`, which runs `k8s:down` then `k8s:up`.
- [x] Write `.github/workflows/kubernetes.yml`, gated on `dorny/paths-filter` rather
...
- [x] Write the filter narrowly at one end and widely enough at the other.
...
- [x] Add the mise task `k8s:render`, which regenerates
      ...
      ".yml"'`. Put the path in the split expression, because `yq -s`
...
- [x] Set `namespace: kanae` on every resource in the chart, and pass
...
- [x] Pin `image.tag` to a digest and let Renovate move it.
      ...
      `IfNotPresent`.
- [x] Add a render workflow that runs `k8s:render` on any pull request touching
      ...
      satisfy.
- [x] Give the other five images the shape Renovate reads. `postgres.image`,
...
- [x] Write the `deploy/kubernetes/tests/e2e.sh` skeleton: create a cluster,
      ...
      the full suite.
- [x] Add `k8s:render:check` to `.github/workflows/kubernetes.yml`, and have CI also
...
- [x] Add a `k8s:validate` task running `kubeconform -strict -summary` over
      `deploy/kubernetes/dist/` and `deploy/kubernetes/*.yml`, and add it to
      `.github/workflows/kubernetes.yml`. Pass
      `-ignore-filename-pattern 'k3d\.yml$'`, since that file is a k3d cluster
      config rather than a Kubernetes manifest.
      kubeconform has no config file, so the flags live in the mise task.
- [x] Add `kube-linter` to `[tools]` and a `k8s:policy` task that runs it over
...
- [x] Leave kube-linter's `unset-cpu-requirements` off, since it demands a
...
- [x] Add a `k8s:policy` check that `deploy/kubernetes/dist/` contains no `kind:
      Secret`. The render pipeline filters them out, but a filter is a line
      somebody can delete.
- [x] Rename `helm:lint` to `k8s:schema`, and add it to
      `.github/workflows/kubernetes.yml`. The current name reads like a quality
...
- [x] Move the "Decisions worth not re-litigating" section of
...
`deploy/kubernetes/dist/deployment-kanae.yml`, and show one changed environment
variable.
...
never passes. Instead `deploy/kubernetes/secrets.sops.yml` holds the
...
- [ ] Check which Borg version the backup image ships, then set
      ...
      `deploy/kubernetes/src/templates/backup-borgmatic.yml` to match what it
...
- [ ] Write the rotation entries. Routine rotation goes through a pull request.
      ...
      `kubectl rollout restart`, then reconcile `secrets.sops.yml` and let
...
- [ ] Create the cluster with the provider's managed ingress add-on turned off,
      ...
      configures.
- [ ] Apply `deploy/kubernetes/storage.yml` from Phase 4. Do this before the
      first install, because the Postgres claim names that class.
- [ ] Apply `deploy/kubernetes/namespace.yml` as cluster setup, not as part of
